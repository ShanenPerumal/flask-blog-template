...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("That email address is already registered on this site.")
        elif User.query.filter_by(username=form.name.data).first():
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("No account with that email is registered.")
